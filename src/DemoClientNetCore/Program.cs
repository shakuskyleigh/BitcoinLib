...
namespace DemoClientNetCore
{
    class Program
    {
        private static readonly ICoinService CoinService = new BitcoinService(true, "http://localhost:8332", "{rpcUsername}", "{rpcPassword}", "{walletPassword}", 60);
        ...
        private static async Task Mains()
        {
            ...
            try
            {
                ...
                //  Current block
                var currblock = await CoinService.GetBlockCountAsync();
                Console.WriteLine("Current block: {0}",currblock.ToString("#,#", CultureInfo.InvariantCulture));
                ...
                if (myBalance > 0)
                {
                    ...
                    //  My private keys
                    if (false && myNonEmptyAddresses.Count > 0 && await CoinService.IsWalletEncryptedAsync())
                    {
                        ...
                        foreach (var address in myNonEmptyAddresses)
                        {
                            Console.WriteLine("Private Key for address " + address.Address + ": " + CoinService.DumpPrivKeyAsync(address.Address));
                        }
